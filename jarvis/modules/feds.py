...
from jarvis.modules.helper_funcs.misc import is_module_loaded, send_to_list
...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @JarvisSupportOT if the problem persist."
            )
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @JarvisSupportOT should this problem persist!"
            )
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @JarvisSupportOT."
            )
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @JarvisSupportOT !"
            )
            return
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if int(import_userid) in SUDO_USERS:
                        failed += 1
                        continue
                    if int(import_userid) in TECHY_USERS:
                        failed += 1
                        continue
